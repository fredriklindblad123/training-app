 ...
 * så man kommer igång utan att först behöva hoppa till Blockplan — men bara
 * grundtypen sätts här (dag + typ, alltid slot 1); rubrik, mål-tid/distans,
 * repgrupper och träningsfaktor per pass fylls i sedan på Blockplan, det är
 ...
 * i efterhand är Blockplans jobb. */
 ...
 * `<select>` som Blockplans "Lägg till pass") är fri att lämna tom, precis
 ...
 * till Blockplan, ta bort-knapp. Delad mellan den enskilda löparens
  ...
  return (
    ...
    >
      ...
      <summary className="flex cursor-pointer flex-col gap-1">
        ...
        {/* Säsongen som eget märke, inte som ett ord i radan. Listan är
            kronologisk — att gruppera den på säsong bröt tidsordningen, se
            SeasonTimeline — så säsongstillhörigheten måste synas per kort.
            Block utan säsong får inget märke: förberedelse och återhämtning
            ligger mellan säsongerna och hör inte till någon. */}
        <span className="flex flex-wrap items-baseline gap-x-2 text-xs text-[var(--ink-3)]">
          <span>
            {PERIOD_LABELS[b.period]} · {PHASE_LABELS[b.phase]}
          </span>
          {b.season && (
            <span className="display rounded-full border border-[var(--line)] px-1.5 py-0.5 text-[10px] font-semibold tracking-[0.06em] text-[var(--ink-2)] uppercase">
              {seasonSectionKey(b).label}
            </span>
          )}
        </span>
...
export default async function ArsplanPage({
  ...
  return (
    <div className="flex flex-1 flex-col gap-8 px-6 py-8">
      ...
      {/* ---------------- Tävlingar ----------------
          Tävlingarna låg tidigare som romber under tidslinjen och var
          oläsbara — ett band som spänner ett år rymmer inte ett tiotal
          tävlingar med namn, prioritet och datum utan att de skriver över
          varandra. Rapporterat två gånger.
      ...
      <section className="flex flex-col gap-3">
        ...
        <p className="mt-1 max-w-3xl text-sm text-[var(--ink-2)]">
          ...
          i på <Link href="/blockplan" className="underline">Blockplan</Link> — ett pass som
