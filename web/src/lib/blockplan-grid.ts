 ...
 * etablerade för Säsongsöversikt-fliken. */
...
export type BlockplanFactorRow = { group: TrainingFactorGroup; subgroup: TrainingFactorSubgroup | null };
...
export function usedBlockplanFactorRows(items: BlockplanItemInput[]): BlockplanFactorRow[] {
  const seen = new Set<string>();
  const rows: BlockplanFactorRow[] = [];
...
export function hasUngroupedBlockplanItems(items: BlockplanItemInput[]): boolean {
 ...
 * datarad. */
export function blockplanRowCountByGroup(
  rows: BlockplanFactorRow[],
...
export function blockplanItemsFor<T extends BlockplanItemInput>(
  items: T[],
  weekday: number,
  row: BlockplanFactorRow | { group: null },
): T[] {
  return items
    .filter(
      (it) =>
